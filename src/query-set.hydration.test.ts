...
describe("query-set: hydration", () => {
	...
	test("hydration: SQL reserved words work as aliases at base and nested levels", async () => {
		// Kysely quotes identifiers, and hydration treats names as plain JS keys,
		// so reserved words flow through both layers untouched.
		const result = await querySet(db)
			.selectAs("user", db.selectFrom("users").select(["id"]).select("users.username as order"))
			.leftJoinMany(
				"posts",
				({ eb, qs }) =>
					qs(eb.selectFrom("posts").select(["id", "user_id"]).select("posts.title as group")),
				"posts.user_id",
				"user.id",
			)
			.where("users.id", "=", 2)
		...
		assert.deepStrictEqual(result, [
			{
				...
				order: "bob",
				posts: [
					{ id: 1, user_id: 2, group: "Post 1" },
					{ id: 2, user_id: 2, group: "Post 2" },
					{ id: 5, user_id: 2, group: "Post 5" },
					{ id: 12, user_id: 2, group: "Post 12" },
				],
			},
		]);
	});
	...
	//
	// extras / omit interaction semantics
	//

	test("extras: do not cascade - each receives the original row", async () => {
		const users = await querySet(db)
			.selectAs("user", db.selectFrom("users").select(["id", "username"]))
			.where("users.id", "=", 1)
			.extras({
	...
	test("omit: removes original fields, not extras", async () => {
		const users = await querySet(db)
			.selectAs("user", db.selectFrom("users").select(["id", "username"]))
			.where("users.id", "=", 1)
			.extras({
	...
	//
	// Collection modification - modifying nested collections via .modify(key, ...)
	//
	...
	test("modify collection: chained where, extras, and omit", async () => {
		const users = await querySet(db)
			.selectAs("user", db.selectFrom("users").select(["id", "username"]))
			.where("users.id", "=", 2)
			.innerJoinMany(
	...
	//
	// Transformation composition across sibling collections
	//

	test("map: composes with sibling collection transformations", async () => {
		const users = await querySet(db)
			.selectAs("user", db.selectFrom("users").select(["id", "username"]))
			.where("users.id", "=", 2)
			.innerJoinOne(
