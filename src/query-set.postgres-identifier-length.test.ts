...
describePg("query-set: postgres identifier length (63-byte truncation)", () => {
	...
	test("orderBy on a nested column whose generated alias exceeds 63 bytes", async () => {
		// The orderBy expression references
		// "theOrganizationalDepartmentEmployingThisEmployeeRecord$$department_name"
		// (72 chars), whose generated alias exceeds Postgres's 63-byte limit.
		// The ORDER BY clause must track whatever the library does to keep the
		// alias legal, and the ordering must actually take effect.
		const records = await querySet(snakeDb)
			.selectAs(
				"record",
				snakeDb
					.selectFrom("departmental_employee_records")
			...
			.innerJoinOne(
				"theOrganizationalDepartmentEmployingThisEmployeeRecord",
				({ eb, qs }) =>
					qs(eb.selectFrom("organizational_departments").select(["id", "department_name"])),
				"theOrganizationalDepartmentEmployingThisEmployeeRecord.id",
				"record.organizational_department_id",
			)
			.orderBy("theOrganizationalDepartmentEmployingThisEmployeeRecord$$department_name", "desc")
			.execute();
		...
		assert.deepStrictEqual(records, [
			{
				...
				theOrganizationalDepartmentEmployingThisEmployeeRecord: {
					id: 2,
					department_name: "Zoology Research",
				},
			},
			{
				id: 4,
	...
	test("orderBy on an over-long nested column with pagination and a many-join", async () => {
		// With pagination plus a many-join, the query is built as a
		// cardinality-one subquery (with the limit and inner ORDER BY) wrapped in
		// an outer query that re-orders by the hoisted column.  Both the inner
		// and outer ORDER BY reference the over-long alias
		// "theOrganizationBehindThisParticularDepartmentalUnit$$organization_name"
		// (71 chars), and the many-join's alias
		// "departmentalEmployeeRecords$$employee_preferred_full_display_name"
		// (65 chars) is over-long as well.
		const departments = await querySet(snakeDb)
			.selectAs(
				"department",
				snakeDb
			...
			.innerJoinOne(
				"theOrganizationBehindThisParticularDepartmentalUnit",
				({ eb, qs }) => qs(eb.selectFrom("organizations").select(["id", "organization_name"])),
				"theOrganizationBehindThisParticularDepartmentalUnit.id",
			...
			.orderBy("theOrganizationBehindThisParticularDepartmentalUnit$$organization_name", "desc")
			.limit(2)
			.execute();
		...
		assert.deepStrictEqual(departments, [
			{
				...
				theOrganizationBehindThisParticularDepartmentalUnit: {
					id: 2,
					organization_name: "Zenith Dynamics",
				},
				departmentalEmployeeRecords: [
					{
						id: 3,
	...
	test("with CamelCasePlugin: orderBy on a nested column whose snake_cased alias exceeds 63 bytes", async () => {
		// The orderBy expression references
		// "theOrganizationalDepartmentEmployingThisEmployeeRecord$$departmentName"
		// (71 chars), which the plugin snake_cases even further past the 63-byte
		// limit.  The ORDER BY must reference whatever alias the library actually
		// emits, and the ordering must take effect.
		const records = await querySet(camelDb)
			.selectAs(
				"record",
				camelDb
					.selectFrom("departmentalEmployeeRecords")
			...
			.innerJoinOne(
				...
				"theOrganizationalDepartmentEmployingThisEmployeeRecord.id",
				"record.organizationalDepartmentId",
			)
			.orderBy("theOrganizationalDepartmentEmployingThisEmployeeRecord$$departmentName", "desc")
			.execute();
		...
		assert.deepStrictEqual(records, [
			{
				...
				theOrganizationalDepartmentEmployingThisEmployeeRecord: {
					id: 2,
					departmentName: "Zoology Research",
				},
			},
			{
				id: 4,
