...
## 5. Resolution (2026-07-11, same day)

Every finding in §2 and the §3.1 test gap has been fixed (or explicitly
documented where a fix was the wrong tradeoff) in one PR branch per
independent fix, listed below. Full
matrix after the fixes: SQLite 587 pass / 0 fail, Postgres 700 pass / 0
fail (2 pre-existing skips each), typecheck and lint clean.

| Findings     | PR branch                               | Resolution                                                                                                                                                                                                                                                                                                                                                                               |
| ------------ | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q1, Q2       | `claude/fix-join-attach-override`       | `has()`/`attach()` now delete the same key from the other map; `with()` applies last-writer-wins across kinds. Joins and attaches fully replace each other; stale fetchFns no longer run.                                                                                                                                                                                                |
| H1           | `claude/fix-extend-proto-pollution`     | Extender results with an own enumerable `__proto__` key are pre-shadowed as an own data property before `Object.assign`, preserving exact merge semantics otherwise.                                                                                                                                                                                                                     |
| Q7           | `claude/fix-sqlcompare-total-order`     | `sqlCompare` is a strict total order: type-rank for cross-type pairs, NaN/invalid Dates pinned after their type, null vs undefined equal. Same-type comparisons unchanged.                                                                                                                                                                                                               |
| Q6, Q8       | `claude/fix-query-modifiers`            | Modifiers applied exactly once on the outermost builder on all six `#toQuery` paths; on the write fast path `modifyEnd` lands on the write builder and `modifyFront` forces the CTE wrap. Redundant `selectAll()` removed.                                                                                                                                                               |
| Q3           | `claude/fix-order-by-nested-many-joins` | Made it work: one-joins that aren't recursively cardinality-one are included in the paginated subquery in reduced form (without their nested many-joins), so ordering by their columns with pagination now executes correctly (incl. lateral top-N). No type change needed. Ordering by a _many_-join's column with pagination remains invalid (pre-existing, semantically ill-defined). |
| Q4, Q9       | `claude/fix-write-cte-hoisting`         | Data-modifying CTEs hoisted to the top-level statement at both wrap sites (pagination and EXISTS — the plain-write `__base` CTE in `toExistsQuery` was also affected); `#asWrite` resets the stale `writeQueryCreator`. `toCountQuery` was verified already correct and pinned.                                                                                                          |
| Q5           | `claude/fix-write-cte-hoisting`         | Write bases with explicit RETURNING columns now hoist those names through the pagination wrap; `returningAll()` gets a clear `UnsupportedReturningAllError` instead of the misleading internal error.                                                                                                                                                                                    |
| H3           | `claude/fix-sort-input-mutation`        | The sort path copies before sorting — fixes caller-array mutation and a second latent bug where a sibling collection's `orderBy` reordered the shared group array other siblings read.                                                                                                                                                                                                   |
| H2, H7       | `claude/fix-key-encoding`               | Composite key parts are self-delimiting tokens (type tag + JSON string literal), provably injective — bigint/string, NaN/Infinity, and Date/ISO collisions eliminated. Single-key raw-value semantics kept and documented (H7).                                                                                                                                                          |
| H6           | `claude/fix-attach-empty-fetch`         | Empty attach inputs skip the fetchFn entirely (per nesting level); `FetchFn` documents it is never called with zero inputs.                                                                                                                                                                                                                                                              |
| H8           | `claude/fix-attach-key-arity`           | Mismatched `matchChild`/`toParent` arity throws `AttachedKeysArityMismatchError` at registration, with `["id"]` ≡ `"id"` normalization.                                                                                                                                                                                                                                                  |
| H9           | `claude/fix-hydrate-factory-throw`      | Standalone `hydrate()` rejects (never throws synchronously) when the factory returns a non-hydrator.                                                                                                                                                                                                                                                                                     |
| H5           | `claude/docs-attach-duplicate-keys`     | Resolved by documented constraint: fetch-input dedupe is global and pre-sort, so "same row for fetch and lookup" would require restructuring the pipeline — duplicate-keyed parents must agree on `toParent` columns (FetchFn JSDoc, with the consistent case pinned by test).                                                                                                           |
| H4, §3.1 gap | `claude/docs-attach-sharing`            | Attach element sharing documented on `FetchFn`, the attach methods, and the README; matchless-left-join `executeExists` assertions restored for `leftJoinOne`/`leftJoinMany`.                                                                                                                                                                                                            |
...
the fixes, both corrected within the affected PR branches: a nullish extender return
